...
function intToRoman(num: number): string {
    ...
    let returnArr =  [];
    let stringArr = ["Z","M","D","C","L","X","V","I"]
    let stringLength = stringArr.length;
    for(let i = 0;i<numLength;i++){
        let curNum =  num%10;
        num = Math.floor(num/10);
        returnArr.push(romeMap(stringArr[stringLength-1-2*i],stringArr[stringLength-2-2*i],stringArr[stringLength-3-2*i],curNum));
    }
